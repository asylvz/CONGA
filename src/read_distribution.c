...
void init_rd_per_chr( bam_info* in_bam, parameters* param, int chr_index)
{
	...
	in_bam->read_depth = ( int*) getMem( sizeof( int) * ( param->genome->chromosome_lengths[chr_index]));
	memset (in_bam->read_depth, 0, (param->genome->chromosome_lengths[chr_index] * sizeof(int)));
}

void init_mappability_per_chr(bam_info* in_bam, parameters* param, int chr_index)
{
	in_bam->mappability = ( float*) getMem( sizeof( float) * ( param->genome->chromosome_lengths[chr_index]));
	memset (in_bam->mappability, 0, (param->genome->chromosome_lengths[chr_index] * sizeof(float)));
}
...
void calc_mean_per_chr( parameters *params, bam_info* in_bam, int chr_index)
{
	...
	calc_mu_per_chr( in_bam, params->genome->chromosome_lengths[chr_index]);
	...
	for( i = 0; i < params->genome->chromosome_lengths[chr_index]; i++)
	{
		if((i + WINDOWSLIDE) < params->genome->chromosome_lengths[chr_index])
			end = i + WINDOWSLIDE;
		else
			end = params->genome->chromosome_lengths[chr_index];

		gc_val = (int) round ( genome_get_gc_content( params->genome, params->genome->chromosome_names[chr_index], i, end));
