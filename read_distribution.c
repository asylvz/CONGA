...
{
	in_bam->total_read_count = 0;
	in_bam->rd = ( short*) getMem( sizeof( short) * ( param->this_sonic->chromosome_lengths[chr_index]));

	memset (in_bam->rd, 0, (param->this_sonic->chromosome_lengths[chr_index] * sizeof(short)));

	// For low qual reads
	in_bam->total_read_count_lq = 0;
	in_bam->rd_lq = ( short*) getMem( sizeof( short) * ( param->this_sonic->chromosome_lengths[chr_index]));

	memset (in_bam->rd_lq, 0, (param->this_sonic->chromosome_lengths[chr_index] * sizeof(short)));
}
...
void calc_mean_per_chr( parameters *params, bam_info* in_bam, int chr_index)
{
	...
	long rd_per_gc[101], rd_per_gc_lq[101];
	...
	for( i = 0; i < params->this_sonic->chromosome_lengths[chr_index]; i++)
	{
		...
		rd_per_gc[gc_val] += ( long) in_bam->rd[i];
		rd_per_gc_lq[gc_val] += ( long) in_bam->rd[i] + ( long) in_bam->rd_lq[i];
		window_per_gc[gc_val]++;
	}

	in_bam->expected_rd[0] = 0.0;
	for( i = 1; i < 101; i++)
	{
		in_bam->expected_rd[i] = ( float)rd_per_gc[i] / ( window_per_gc[i]);

		if( isnanf( in_bam->expected_rd[i]) || isinff( ( in_bam->expected_rd[i])) == -1
				|| isinff( ( in_bam->expected_rd[i])) == 1 )
			in_bam->expected_rd[i] = 0;
		fprintf(stderr,"GC = %d - RD=%ld\tWINDOW=%d\tMEAN=%f\n", i, rd_per_gc[i], window_per_gc[i], in_bam->expected_rd[i]);
	}

	in_bam->expected_rd_lq[0] = 0.0;
	for( i = 1; i < 101; i++)
	{
		in_bam->expected_rd_lq[i] = ( float)rd_per_gc_lq[i] / ( window_per_gc[i]);

		if( isnanf( in_bam->expected_rd_lq[i]) || isinff( ( in_bam->expected_rd_lq[i])) == -1
				|| isinff( ( in_bam->expected_rd_lq[i])) == 1 )
			in_bam->expected_rd_lq[i] = 0;
	}
}
